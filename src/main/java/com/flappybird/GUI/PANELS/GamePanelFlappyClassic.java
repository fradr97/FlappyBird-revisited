...
public class GamePanelFlappyClassic extends JPanel {
    ...
    private static final int x = 250; //spazio tra le colonne
    ...
    public GamePanelFlappyClassic(FlappyBirdFrame pFlappyBirdFrame) {

        this.setSize(FlappyBirdFrame.WIDTH_FRAME, FlappyBirdFrame.HEIGHT_FRAME);
        this.setLayout(null);

        this.addMouseListener(new MyMouseListener());
        this.addMouseListener(new MyMouseStart());

        this.flappyBirdFame = pFlappyBirdFrame;

        this.statusBarPanel = new StatusBarPanel(pFlappyBirdFrame);
        this.statusBarPanel.setBounds(0, 0, FlappyBirdFrame.WIDTH_FRAME, 40); //setBounds è l'aggergazione di setSize e setLocation
    ...
    public void startDown() {
        ...
        Thread down = new Thread(new ThreadDown());
        down.start();
    }
    ...
    public void startTimer() {
        this.statusBarPanel.startTimer();
    }

    public void stopTimer() {
        this.statusBarPanel.stopTimer();
    }
    ...
    public void addColumnBird() {
        ...
        for (int i = 0; i < dimensione; i++) {
            if (i % 2 == 0) {
                ...
                this.column[i] = new Column(image_columnS, FlappyBirdFrame.WIDTH_FRAME + 100 + x * (i - 1), 0, 100, FlappyBirdFrame.HEIGHT_FRAME - altezza - space);
                altezza = 50 + rand.nextInt(300);
            }
        }
    }
    ...
    public void scoreCounts(){
        ...
        for (int i = 0; i < dimensione; i++) {
            ...
            if ((column[i].y == 0) && (bird.x + bird.width / 2 > column[i].x + column[i].width / 2 - 15) && (bird.x + bird.width / 2 < column[i].x + column[i].width / 2)) {
                game.increaseScore();
            }
        }
    ...
    public void checkGameOver() throws SQLException {
        if (this.collision() == true) {
            this.statusBarPanel.pause.isActive(false); //disattivo il pulsante pause
            flappyBirdFame.switchPanel(flappyBirdFame.endPanel, flappyBirdFame.gamePanelFlappyClassic);
    ...
    private class MyMouseStart extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (start.isClicked(e)) {
                ...
                sound.play();
                statusBarPanel.pause.isActive(true);
